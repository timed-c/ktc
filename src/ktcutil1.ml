...
module AV = struct
		...
		List.iter (fun f -> ignore (E.log "Pred %d -- %d\n" s.sid f.sid)) s.preds;
...
let checkAvail data s il =
	if not(computeAvailOfStmt data s il) then 
                        E.s (E.bug "TIME-C: Variable in use not availble for %d: \n%a" s.sid d_stmt s) 
	...
	let tsuccsofs = getStmtTPSucc data s in
		let firmsucc = TS.filter retFirm tsuccsofs  in
			if TS.cardinal firmsucc > 1 then 
				 E.s (E.bug "TIME-C: Conflicting firm delay %d: \n%a" s.sid d_stmt s)
