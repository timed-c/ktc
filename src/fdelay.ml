...
let makeFdelayInitInstr (structvar : varinfo) (argL : exp list) (loc : location) (timervar :varinfo) : instr list =
  let time_unit = if (L.length argL) = 1 then mkString "NULL" else L.hd (L.tl argL) in
  let f, l, intervl, tunit, s, t_id = mkString loc.file, integer loc.line, L.hd argL, time_unit, mkAddrOf((var structvar)), (List.hd(List.rev argL)) in
  [Call(None,v2e sdelayfuns.fdelay_init, [intervl;tunit;s;(v2e timervar); t_id;], loc)]
...
let makeTaskCreateInstr (handlevar : varinfo) (funvar : varinfo) (idlePrioVar : varinfo) (loc:location) arg= 
	...
	Call(None, v2e sdelayfuns.task_create, [v2e funvar; (mkString (funvar.vname^handlevar.vname)); (integer 190); (List.hd arg) ; v2e idlePrioVar; addrHandle], loc)
...
class sdelayReportAdder filename fdec structvar jmpvar timervar ret_jmp data fname   = object(self)
  inherit nopCilVisitor
       ...
       method vinst (i :instr) =
        ...
        |Call(_,Lval(Var vi,_),argList,loc) when (vi.vname = sname) -> makeFdelayInitInstr structvar argList loc timervar
	(*|Call(_,LVal(Var vi,_),_,loc) when (vi.vname = "next") -> makeNextGoto loc *)
	|Call(_,Lval(Var vi,_),arg,_) when (isFunTask vi) ->
