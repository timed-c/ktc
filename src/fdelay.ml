...
let calculateHyperperiod tlist =
    ...
    let max_arrival = List.fold_right max_arr (!offsetlist) 0 in
    E.log "Hyperperiod %d\n" ((hyperiod * 2) + max_arrival); ((hyperiod * 2) +
    max_arrival)
...
let rec unrolledJob num p j b w d pr nnow =
    ...
    match nnow with
    | t when t = num -> []
    | 0 -> ([pr; 0; 0; j; b; w; d; d]) :: (unrolledJob num p j b w d pr (nnow  + 1))
    (*|  _ -> let md = (nnow mod num) in
    if (md <> 1) then
    (([pr; (md); md*p; ((md*p)+j); b; w; (md*p + d);pr]) :: (unrolledJob num
    p j b w d pr (nnow -1))) else [] :: (unrolledJob num p j b w d pr (nnow -1))*)
    |  _ ->  ([pr; (nnow); nnow*p; ((nnow*p)+j); b; w; (nnow*p + d);  nnow*p + d]) :: (unrolledJob num
    p j b w d pr (nnow + 1))
...
let unrollSegment telem next_d jlist id  jid =
    ...
    (int_of_string w);  (p_int + next_d_int); (p_int + next_d_int)]
...
let unrollMultiFrameAux tlist hp id tname =
    ...
    let [n1; p1; j1; b1; w1; d1] = List.hd tlist in
    let nlst = List. rev (List.tl (recUnrollSegment tlist hp id tname iter [])) in
    [(int_of_string id); 0; 0;( int_of_string j1);
    (int_of_string b); (int_of_string w); (int_of_string d1); (int_of_string d1)] :: nlst
...
let addJsonNodes jnodes arrival_time deadline kind tname d =
    ...
    (string_of_int bcet); (string_of_int wcet); (string_of_int deadline)]) ::
        !csvlist) else offsetlist := (arrival_time) :: !offsetlist);
