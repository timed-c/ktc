...
class profilingAdder filename logname_var lastarrival stime itime fdec id_var count_var loop_var flogvar mkarray_var mkmisses_var mkcounter_var = object(self)
    ...
    let action [i] =
        ...
        |Call(_,Lval(Var vi,_),argList, loc) when (isFunTask vi) ->
                let logstart = findType filename.globals "TickType_t" in
                ...
                logstart) in
                [i]
        |Call(_,Lval(Var vi,_),argList, loc) when ((vi.vname = "sdelay" || vi.vname = "fdelay") & (fdec.svar.vname <> "main")) ->
...
class profileTask filename = object(self)
    ...
    method vfunc (fdec : fundec) =
        ...
        (*print release*)
        let execution_start_var = makeLocalVar fdec "ktcstime" timestruct in
...
profileTransformation f;timingConstructsTransformatn f; (*concurrencyConstructsTransformatn f ;*) ()
