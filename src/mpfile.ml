...
let log_trace_previous_id_str = "mplog_trace_end_id"
let log_trace_fclose_str = "fclose"
let log_trace_final_file_str = "mplog_write_to_file"
let log_trace_init_str = "fopen"
let log_trace_init_task_str = "mplog_trace_init_tp"
let log_trace_arrival_str = "mplog_trace_arrival"
let log_trace_execution_str = "mplog_trace_execution"
...
let makeLogTraceFinalFile filepointer buf misses fname loc =
    Call(None, v2e sdelayfuns.log_trace_final_file, [filepointer;buf;misses; fname], loc)
...
let makeLogTraceTask filepointer last_arrival itime fp loc =
    Call(None, v2e sdelayfuns.log_trace_init_task, [filepointer; fp; Cil.zero; last_arrival; itime], loc)
...
let makeLogTraceAbortTime mm_pointer ls_pointer dl res mkarr mkmiss mkcount loc =
    Call(None,v2e sdelayfuns.log_trace_abort_time, [mm_pointer; ls_pointer; dl; res; mkarr; mkmiss; mkcount], loc)
...
let makeCriticalStartInstr (signum) cwcet (loc: location) =
	(*let argSig = mkAddrOf (var sigvar) in *)
	i2s	(Call(None, v2e sdelayfuns.critical_start, [signum;cwcet], loc) )

let makeCriticalEndInstr (signum) cwcet retwcet (loc: location)=
      (*  let argSig = mkAddrOf (var sigvar) in*)
    i2s ( Call(None, v2e sdelayfuns.critical_end, [signum;cwcet;retwcet], loc) )
...
let maketimerfdelayStmt structvar argList tpstructvar timervar retjmp firmStmt timer_creater =
    ...
    [mkStmtOneInstr timer_creater; mkStmtOneInstr startTimer; mkStmtOneInstr sigInt; ifBlock; waitingConditionStmt]
    ...
    let action [i] =
        ...
        vi.vname = "ktc_fdelay_init_profile") & (fdec.svar.vname <> "main")) -> (*if
            (isdeadlineInfinity argList < 2147483640) then *)
               ...
               let loop_instr = makeLogTraceAbortTime ((v2e minmaxlog_var)) (mkAddrOf (var logname_var)) (List.nth argList 0) (List.nth argList 2) (v2e mkarray_var) (mkAddrOf (var mkmisses_var))
    ...
    let action [i] =
        ...
        |Call(_,Lval(Var vi,_),argList, loc) when ((vi.vname = "sdelayi" || vi.vname = "fdelayi") & (fdec.svar.vname <> "main")) ->
               ...
               let trace_end_instr = makeLogTraceExecution    (mkAddrOf (var logname_var)) (v2e stime) (mkAddrOf (var itime)) locUnknown in
                ...
                let loop_instr = makeLogTraceAbortTime ((v2e flogvar)) (mkAddrOf (var logname_var)) (List.nth argList 0) (List.nth argList 1) (v2e mkarray_var) (mkAddrOf (var mkmisses_var)) (mkAddrOf (var mkcounter_var)) locUnknown in
...
*)
	...
	|If(CastE(t,z),b,_,_) when isCriticalType t ->  begin
                            let cwcet = findLocalVar fdec.slocals ("ktc_cwcet") in
                            let fcwcet = findLocalVar fdec.slocals ("ktc_ret_cwcet") in
							let cs_start = makeCriticalStartInstr (integer signo) (mkAddrOf (var cwcet)) (get_stmtLoc s.skind) in
							let cs_end = makeCriticalEndInstr (integer signo) (v2e cwcet) (mkAddrOf (var fcwcet)) (get_stmtLoc s.skind) in
...
class sdelayFunc filename fname fno = object(self)
	...
	(*	Cfg.cfgFunPrint (fdec.svar.vname^".dot") fdec;
        ...
        let critical_wcet = makeLocalVar fdec "ktc_cwcet" (TComp(ci,[])) in
        let critical_ret_wcet = makeLocalVar fdec "ktc_ret_cwcet" intType in
...
class profileTask filename = object(self)
    ...
    method vfunc (fdec : fundec) =
        ...
        let add_end_instr_without_return = List.rev (List.append ([mkStmtOneInstr trace_end_instr]) (List.tl (List.rev fdec.sbody.bstmts))) in
