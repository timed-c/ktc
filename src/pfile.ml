...
let makeFdelayInitInstr fdec (structvar : varinfo) (argL : exp list) (loc :
    location) (retjmp) (signo) tpstructvar lv : instr list  =
  ...
  [deadline;period;tunit;s;t_id;(v2e retjmp); signo; ((mkAddrOf ((Var(logname_var), Index(v2e loop_var, NoOffset))))); v2e count_var], loc)]
...
class profileTask filename = object(self)
    ...
    method vfunc (fdec : fundec) =
        ...
        (TArray((TComp(logname,[])), Some((integer 100)), [])) in
