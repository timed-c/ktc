...
let findgoto data s = 
	 try IH.find data s.sid
  		with Not_found -> E.s(E.bug "%d jgd" s.sid)
...
class correctnessAnalysis fdec = object(self)
	...
	|Instr (il) ->  
			...
			let label_no = s.sid + 2 in
			let label_name = "_"^string_of_int(label_no) in
			let label_stmt = mkStmt (Instr []) in 
                		label_stmt.labels <- [Label(label_name,locUnknown,false)]; E.log "%d" label_no; IH.add labelHash label_no label_stmt; 
			let changStmTo = [label_stmt; mkStmt s.skind] in
			let block = mkBlock  changStmTo in
                	s.skind <- Block block ;
                	s
                	else
...
initSdelayFunctions f;   Cfg.computeFileCFG f;isErrorFree f; Cfg.clearFileCFG f;  timing_basic_block f;
let fname = "sdelay" in
